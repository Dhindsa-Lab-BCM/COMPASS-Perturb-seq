...
so <- merge(x = so_list[[1]], y = so_list[2:length(so_list)],
            add.cell.ids = c("compass_iNeuron", "compass_iPSC", "NDD_iNeuron", "NDD_iPSC"))
...
```{r}
# PCA Plot
pca.plot <- DimPlot(so, reduction = "pca", group.by = c("cell_type", "sequencing_run"))
print(pca.plot)
...
before_integration_umap <- DimPlot(so, reduction = "umap.unintegrated", group.by = c("cell_type", "sequencing_run"))
...
#qs_save(object = so, file = paste0(dir_10x, "seurat_objects/1_integrated_processed_NTC.qs2"))
