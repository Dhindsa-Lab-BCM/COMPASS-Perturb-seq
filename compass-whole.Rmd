...
so <- qs_read(file = paste0(dir_10x, "seurat_objects/1_COMPASS_integrated_processed.qs2"))
```
...
pp_dist <- cleanser_posterior %>% 
  ggplot(aes(x = log10(posterior_probability))) + 
  geom_histogram(binwidth = 0.1, fill = "lightblue", color = "black") + 
  geom_vline(xintercept = c(log10(0.5)), color = "red", linetype = "dashed", linewidth = 1) + 
  scale_x_continuous(breaks = seq(floor(min(log10(cleanser_posterior$posterior_probability), na.rm = TRUE)), 0, by = 1)) +
  scale_y_continuous(expand = c(0, 0)) +
  labs(x = "log10(Posterior Probability)", y = "sgRNA Assignment Counts", title = "sgRNA Distribution of Posterior Probabilities") + 
...
# Saving plots
#ggsave(plot = pp_dist, filename = "iNeuron_figures/CLEANSER_posterior_probabilities_histogram.pdf", width = 3000, height = 2000, units = "px", dpi = 300)
...
num.grna.hist <- num.grna.calls %>% ggplot(aes(x = Var1, y = Freq)) + 
  ...
  labs(x = "Number of sgRNAs per Cell", y = "Number of Cells", title = "Number of sgRNA Assignments per Cell") + 
  theme_classic(base_size = 20) + 
  theme(
    plot.title = element_text(hjust = 0.5, size = rel(1.2)),
    axis.text = element_text(color = "black")
...
# Saving plot
#ggsave(plot = num.grna.hist, filename = "iNeuron_figures/CLEANSER_guide_counts_histogram.pdf", width = 2500, height = 2000, units = "px", dpi = 300)
```
...
#qs_save(object = so, file = paste0(dir_10x, "seurat_objects/1_COMPASS_integrated_processed.qs2"))
```
...
so <- qs_read(file = paste0(dir_10x, "seurat_objects/1_integrated_processed_compass.qs2"))
meta <- so@meta.data
```
...
```{r}
# iNeuron or iPSC
so_filt <- subset(so_filt, cell_type == "iPSC")  #change iPSC or iNeuron
...
degs_target_filt <- degs_target_wilcox %>% dplyr::filter(avg_log2FC <= log2(0.7) & FDR < 0.05)

# Saving the list of DEGs
#qs_save(object = full_degs_list, file = "integrative_tables/iPSC_DEGs_wilcox.qs2")  #iPSC or iNeuron
#write_xlsx(x = degs_target_wilcox, path = "integrative_tables/iPSC_target-DEGs_wilcox.xlsx")
...
select_targets <- c("KMT2A", "KMT2B", "KMT2C", "KMT2D", "SETBP1", "SETD1A", "SETD1B")
gRNA_effectiveness_plot <- degs_target_wilcox_total %>% dplyr::filter(target_gene %in% select_targets) %>% 
  dplyr::filter(cell_type == "iNeuron") %>%   #optional cell type filter
  ggplot(aes(x = gene, y = percent_change, color = regulation)) +
  geom_hline(yintercept = c(0, -50), color = "darkgrey", linetype = "dashed", linewidth = 1) + 
  geom_point(size = 3.5, alpha = 0.8) + 
  #geom_text_repel(size = 3, max.overlaps = Inf, show.legend = FALSE) + #label = guide_replicate, 
  #facet_wrap(~cell_type, scales = "free_y", nrow = 2) +   #optional facet_wrap for multiple cell types
  coord_cartesian(ylim = c(-100, 20)) +
  scale_color_manual(values = c("blue", "lightgrey")) + 
  labs(x = "Target Gene", y = "Percent Knockdown", color = "Regulation", title = "Target Gene Knockdown per sgRNA") + 
  theme_classic(base_size = 20) + 
    theme(
      strip.background = element_blank(),
      axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1), # 45 degrees is often easier to read than 90
      plot.title = element_text(hjust = 0.5, size = rel(1.2)),
      legend.position = "top"
    )
...
# Saving the plot
ggsave(plot = gRNA_effectiveness_plot, filename ="integrative_figures/Wilcox_DEGs_per_gRNA_iNeuron.png", width = 2200, height = 2200, units = "px", dpi = 300)
```

```{r}
# Finding the overlapping gRNAs and the unique ones between iNeurons and iPSCs
iPSC_guides <- degs_target_wilcox_iPSC %>% dplyr::filter(FDR < 0.05 & avg_log2FC < -0.2 & cell_count >= 20)
...
iNeuron_guides <- degs_target_wilcox_iNeuron %>% dplyr::filter(FDR < 0.05 & avg_log2FC < -0.2 & cell_count >= 20)
...
gRNA_scatter <- w_wide %>% ggplot(aes(x = iPSC, y = iNeuron, color = target_gene)) + 
  ...
  labs(x = "iPSC: log2(FC)", y = "iNeuron: log2(FC)", color = "Target Gene", title = "Concordance in gRNAs between iPSCs and iNeurons", subtitle = "R-squared = 0.584; p < 2.2e-16") + 
  theme_classic() + 
  theme(plot.title = element_text(hjust = 0.5),
...
## 7.2 Permutation testing - optional can skip

```{r}
# iNeuron or iPSC
so_filt <- subset(so, cell_type == "iPSC")

# Optional shuffling of guide labels for permutation testing
set.seed(1)
so_filt$NT_gRNA_shuffle <- sample(as.character(so_filt$NT_gRNA))
Idents(so_filt) <- "NT_gRNA_shuffle"
guides <- unique(so_filt$NT_gRNA_shuffle)
guides <- guides[guides != "NonTargeting"]
gRNA_counts <- table(so_filt$NT_gRNA_shuffle)
n_total <- length(guides)
n_start = 1
full_shuffle_list <- list()
target_gene_shuffle_list <- list()
...
for (gRNA in guides) {
  ...
  # Adding to list
  full_shuffle_list[[gRNA]] <- degs_temp
  
  # Filtering for target gene degs only
  gene <- gsub("-g[0-9]+$", "", gRNA)
  degs_temp_filt <- degs_temp[degs_temp$gene == gene, ]
  target_gene_shuffle_list[[gRNA]] <- degs_temp_filt
...
```{r}
full_degs_df <- dplyr::bind_rows(full_degs_list)
full_degs_df$gene_gRNA <- paste0(full_degs_df$gene, "_", full_degs_df$gRNA)
full_degs_df <- full_degs_df %>% dplyr::select(gene_gRNA, p_val)
colnames(full_degs_df) <- c("gene_gRNA", "p_val_observed")
full_degs_df$real <- "observed"

full_shuffle_df <- dplyr::bind_rows(full_shuffle_list)
full_shuffle_df$gene_gRNA <- paste0(full_shuffle_df$gene, "_", full_shuffle_df$gRNA)
full_shuffle_df <- full_shuffle_df %>% dplyr::select(gene_gRNA, p_val)
colnames(full_shuffle_df) <- c("gene_gRNA", "p_val_expected")
full_shuffle_df$real <- "expected"

deg_comp <- merge(x = full_degs_df, y = full_shuffle_df, by = "gene_gRNA", all = TRUE)
```

```{r}
# Plotting a permutation qq plot
deg_plot_qq <- deg_comp %>% ggplot(aes(x = -log10(p_val_expected), y = -log10(p_val_observed))) + 
  geom_point(alpha = 0.5) + 
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
  scale_y_continuous(breaks = seq(0, max(-log10(deg_comp$p_val_observed), na.rm = TRUE) + 5, by = 5)) +
  coord_cartesian(xlim = c(0, 25)) + 
  labs(x = "-log10(permuted p value)", y = "-log10(observed p value)", title = "Permutation QQ Plot") + 
  theme_classic() + 
  theme(plot.title = element_text(hjust = 0.5))
print(deg_plot_qq)

# Saving the plot
#ggsave(plot = deg_plot_qq, filename ="integrative_figures/permuted_deg_qq_plot.png", width = 2500, height = 2600, units = "px", dpi = 300)
```

```{r}
# Histograms
ex_histo <- deg_comp %>% ggplot(aes(x = -log10(p_val_expected))) + 
  geom_histogram(binwidth = 1) + 
  scale_y_log10(expand = c(0, 0)) +
  theme_classic()
print(ex_histo)

ob_histo <- deg_comp %>% ggplot(aes(x = -log10(p_val_observed))) + 
  geom_histogram(binwidth = 1) + 
  scale_y_log10(expand = c(0, 0)) +
  theme_classic()
print(ob_histo)
...
deg_comp_long <- rbind(full_degs_df, full_shuffle_df)
deg_comp_long$real <- factor(x = deg_comp_long$real, levels = c("expected", "observed"))

density_plot <- deg_comp_long %>% ggplot(aes(x = -log10(p_val), fill = real)) + 
  geom_density(alpha = 0.5) +
  #scale_y_continuous(trans = "pseudo_log", breaks = c(0, 0.1, 1, 5, 10)) +
  labs(x = "-log10(p value)", y = "Density", fill = "Permuted") + 
  theme_classic()
print(density_plot)
```

```{r}
ob_histo <- deg_comp_long %>% 
  ggplot(aes(x = -log10(p_val), fill = real)) + 
  geom_histogram(binwidth = 1, position = "identity", alpha = 0.4) +
  scale_y_log10(expand = c(0, 0)) +
  theme_classic() +
  labs(y = "Log10 Count of Comparisons", x = "-log10(p-value)", fill = "Permuted")
...
```{r}
# Exporting a tsv
deg_comp_long <- deg_comp_long %>% tidyr::separate(col = gene_gRNA, into = c("gene", "gRNA"), sep = "_", remove = FALSE)
#write.table(x = deg_comp_long, file = "integrative_tables/permute_vs_observed_p-values.tsv", sep = "\t", row.names = FALSE, quote = FALSE)
```



## 7.3 Weighted Mixscale Differential Expression per gRNA - optional
...
# Finding the overlapping gRNAs and the unique ones between iNeurons and iPSCs
iPSC_guides <- degs_target_wilcox_iPSC %>% dplyr::filter(FDR < 0.05 & avg_log2FC < -0.2 & cell_count >= 25)
gRNA_counts <- table(iPSC_guides$gene)
gRNA_counts <- gRNA_counts[gRNA_counts > 1]
iPSC_guides <- iPSC_guides %>% dplyr::filter(gene %in% names(gRNA_counts)) %>% dplyr::pull(gRNA)

iNeuron_guides <- degs_target_wilcox_iNeuron %>% dplyr::filter(FDR < 0.05 & avg_log2FC < -0.2 & cell_count >= 25)
gRNA_counts <- table(iNeuron_guides$gene)
gRNA_counts <- gRNA_counts[gRNA_counts > 1]
iNeuron_guides <- iNeuron_guides %>% dplyr::filter(gene %in% names(gRNA_counts)) %>% dplyr::pull(gRNA)
...
qs_save(object = so_filt, file = paste0(dir_10x, "seurat_objects/2_COMPASS_integrated_MixScale_filtered.qs2"))

# Reading the processed seurat object with Mixscale scores and filtered gRNAs
#so_filt <- qs_read(file = paste0(dir_10x, "seurat_objects/2_integrated_Mixscale_filt_compass.qs2"))
```
...
# Saving the plot
#ggsave(plot = ridge_iNeuron, filename = "integrative_figures/gene_ridge_plot_iNeuron.pdf", width = 2000, height = 1500, dpi = 300, units = "px")
```
...
mixed_scatter <- Mixscale_ScatterPlot(object = so_filt, 
                     nt.class.name = "NonTargeting",
                     slct.ident = unique(so_filt$target_gene)[unique(so_filt$target_gene) != "NonTargeting"][1:10], 
                     nbin = 10,
                     facet_wrap = "target_gene") + NoLegend()
print(mixed_scatter)
...
for (gene in names(de_res_gene)) {
  ...
  # iNeuron regulation
  df$FDR_iNeuron <- p.adjust(p = df$p_weight_iNeuron, method = "fdr")
  df <- df %>% dplyr::mutate(
    regulation_iNeuron = dplyr::case_when(
      FDR_iNeuron < 0.05 & log2FC_iNeuron >= 0.2 ~ "up",
      FDR_iNeuron < 0.05 & log2FC_iNeuron <= -0.2 ~ "down"  
    )
  )
  ...
  # iPSC regulation
  df$FDR_iPSC <- p.adjust(p = df$p_weight_iPSC, method = "fdr")
  df <- df %>% dplyr::mutate(
    regulation_iPSC = dplyr::case_when(
      FDR_iPSC < 0.05 & log2FC_iPSC >= 0.2 ~ "up",
      FDR_iPSC < 0.05 & log2FC_iPSC <= -0.2 ~ "down"  
    )
  )
...
```{r}
# Number of DEGs per target gene
genes_to_analyze <- c("SETD1B", "KMT2B", "KMT2A", "SETBP1", "KMT2C")  #"SETD1A"
de_merge <- dplyr::bind_rows(de_list2) %>% dplyr::filter(target_gene %in% genes_to_analyze)
de_merge <- table(de_merge$target_gene, de_merge$regulation_iNeuron) %>% as.data.frame()
colnames(de_merge) <- c("target_gene", "regulation", "degs")
de_merge <- de_merge %>%  dplyr::mutate(plot_val = ifelse(regulation == "down", -degs, degs))
de_merge$target_gene <- factor(x = de_merge$target_gene, levels = genes_to_analyze)

mirror_plot <- ggplot(de_merge, aes(x = target_gene, y = plot_val, fill = regulation)) +
  geom_col(width = 0.7) +
  geom_hline(yintercept = 0, color = "black") +
  geom_text(data = subset(de_merge, regulation == "up"), aes(label = degs), vjust = -0.5, size = 5) +
  geom_text(data = subset(de_merge, regulation == "down"), aes(label = degs), vjust = 1.5, size = 5) +
  scale_fill_manual(values = c("up" = "#C0392B", "down" = "#2471A3")) +
  scale_y_continuous(labels = abs, limits = c(-1300, 2200)) +
  labs(y = "Number of DEGs", x = "Target Gene") +
...
# Saving the plot
#ggsave(plot = mirror_plot, filename = "integrative_figures/iNeuron_mirror_DEGs.png", width = 2000, height = 2000, units = "px", dpi = 300)
```
...
## 14.3 Upset Plot for Shared vs Divergent DEGs

```{r}
# Up or downregulated, irrespective of regulation direction
genes_to_analyze <- c("KMT2A", "KMT2B", "KMT2C", "SETBP1", "SETD1B")  #"SETD1A"
de_merge <- dplyr::bind_rows(de_list2)

# Regulation in iNeurons
de_results_summary <- de_merge %>%
  dplyr::filter(regulation_iNeuron %in% c("up", "down")) %>%  #iNeuron or iPSC - up or down
  dplyr::group_by(target_gene) %>%  #optional to group by regulation direction
  dplyr::summarise(
    n_DEGs = n(),
    gene_list = list(gene)
  )

de_list_summary <- list("KMT2A" = unlist(de_results_summary %>% dplyr::filter(target_gene == "KMT2A") %>% dplyr::pull(gene_list)),
                        "KMT2B" = unlist(de_results_summary %>% dplyr::filter(target_gene == "KMT2B") %>% dplyr::pull(gene_list)),
                        "KMT2C" = unlist(de_results_summary %>% dplyr::filter(target_gene == "KMT2C") %>% dplyr::pull(gene_list)),
                        "SETBP1" = unlist(de_results_summary %>% dplyr::filter(target_gene == "SETBP1") %>% dplyr::pull(gene_list)),
                        "SETD1B" = unlist(de_results_summary %>% dplyr::filter(target_gene == "SETD1B") %>% dplyr::pull(gene_list)))
                        #"SETD1A" = unlist(de_results_summary %>% dplyr::filter(target_gene == "SETD1A") %>% dplyr::pull(gene_list)),
```

```{r}
m1 <- make_comb_mat(de_list_summary)

#png(file = "integrative_figures/iNeuron_upset_DEG_upset_ALL.png", width = 2500, height = 1200, units = "px", res = 300)
#pdf(file = paste0("iNeuron_figures/upset/statistical_DEG_upset_", tg, ".pdf"), width = 8, height = 4.5)
UpSet(m1,
      set_order = c("KMT2A", "KMT2B", "KMT2C", "SETBP1", "SETD1B"), 
      comb_col = c("black", "#5993ff", "#16b884", "#ffa600", "#ff0000")[comb_degree(m1)], # Color by degree
      ...
      column_title = "Overlap of DEGs across Perturbations: All DEGs")
#dev.off()
```
...
# Create a list where each element contains the genes for that specific overlap
overlap_list <- lapply(names(comb_sizes), function(x) extract_comb(m1, x))
names(overlap_list) <- comb_names

# All 5 overlap
genes_overlap <- extract_comb(m1, '11111')
genes_to_analyze <- c("KMT2A", "KMT2B", "KMT2C", "SETBP1", "SETD1B")  #"SETD1A"
de_merge <- dplyr::bind_rows(de_list2) %>% dplyr::filter(target_gene %in% genes_to_analyze) %>% dplyr::filter(gene %in% genes_overlap)

# grouping by absolute log2FC in iNeurons
de_merge <- de_merge %>% dplyr::group_by(gene) %>% 
  dplyr::mutate(avg_log2FC = mean(abs(log2FC_iNeuron))) %>% 
  dplyr::arrange(desc(avg_log2FC), gene)
top_genes <- unique(de_merge$gene)[1:25]
de_merge <- de_merge[de_merge$gene %in% top_genes, ]

# Plotting
levels_list <- unique(de_merge$gene)
shading_data <- data.frame(
  ymin = seq(1, length(levels_list), by = 2) - 0.5,
  ymax = seq(1, length(levels_list), by = 2) + 0.5
)

deg_dotplot <- de_merge %>% ggplot(aes(x = log2FC_iNeuron, y = reorder(gene, avg_log2FC), color = target_gene)) +
  ...
  geom_jitter(alpha = 0.5, size = 3, width = 0, height = 0.2) +
  #stat_summary(fun = mean, geom = "point", shape = 18, size = 5, color = "black") +
  labs(x = expression(log[2] * "(Fold Change)"), y = "", title = "Top 25 Shared Differentially Expressed Genes", color = "Target Gene") +
  theme_classic(base_size = 20) + 
  theme(
...
# Saving the plot
#ggsave(plot = deg_dotplot, filename = "integrative_figures/iNeuron_top_shared_DEGs.png", width = 3000, height = 3000, units = "px", dpi = 300)
```
...
genes_to_analyze <- c("KMT2A", "KMT2B", "KMT2C", "SETD1A", "SETD1B")

de_merge <- dplyr::bind_rows(de_list2)

# Regulation in iNeurons
de_results_summary <- de_merge %>%
  dplyr::filter(regulation_iNeuron %in% c("up")) %>%  #iNeuron or iPSC - only down or up
  dplyr::group_by(target_gene) %>%  #optional to group by regulation direction
  dplyr::summarise(
    n_DEGs = n(),
    gene_list = list(gene)
...
#png(file = "integrative_figures/iNeuron_upset_DEG_upset_UP.png", width = 2500, height = 1500, units = "px", res = 300)
#pdf(file = paste0("iNeuron_figures/upset/statistical_DEG_upset_", tg, ".pdf"), width = 8, height = 4.5)
UpSet(m1,
      set_order = c("KMT2A", "KMT2B", "KMT2C", "SETD1A", "SETD1B"), 
      comb_col = c("red", "#377EB8", "#4DAF4A", "orange", "purple")[comb_degree(m1)], # Color by degree
      top_annotation = upset_top_annotation(m1, add_numbers = TRUE, annotation_name_side = "left"),
      right_annotation = upset_right_annotation(m1, add_numbers = TRUE),
      column_title = "Overlap of Upregulated DEGs across Perturbations")  #down or upregulated
#dev.off()
```
...
reac <- fgsea::gmtPathways("pathways/enrichment/c2.cp.reactome.v2026.1.Hs.symbols.gmt")
gobp <- fgsea::gmtPathways("pathways/enrichment/c5.go.bp.v2026.1.Hs.symbols.gmt")
```

```{r}
# Custom genomics england gene lists
ge_files <- Sys.glob("pathways/genomics_england/*.tsv")
ge_list <- list()
for (f in ge_files) {
  filename <- tools::file_path_sans_ext(basename(f))
  #print(filename)
  df <- read.csv(file = f, sep = "\t")
  genes <- df$Gene.Symbol
  ge_list[[filename]] <- genes
}

# Making a dataframe
ge_df <- enframe(ge_list, name = "ge_pathway", value = "genes") %>% unnest(genes)

# Filtering the list for only relevant NDD pathways for COMPASS genes
ge_list <- ge_list[c("Autism Spectrum Disorder", "Intellectual disability", "Early onset or syndromic epilepsy", "Childhood onset dystonia, chorea or related movement disorder", "Cerebral malformation", "Severe microcephaly")]
names(ge_list) <- c("Autism Spectrum Disorder", "Intellectual Disability", "Early Onset Epilepsy", "Childhood Dystonia", "Cerebral Malformation", "Severe Microcephaly")

#write.csv(x = ge_df, file = "pathways/genomics_england_gene_lists.csv", row.names = FALSE)
```
...
for (target_gene in names(de_list2)) {
  ...
  gsea_ineuron <- fgsea(pathways = gobp,
...
```{r}
# Saving the file
#qs_save(object = gsea_list, file = "integrative_tables/fgsea_results_list_GOBP.qs2")
```

```{r}
gsea_results_full <- dplyr::bind_rows(gsea_list)
gsea_results_full <- gsea_results_full %>% dplyr::mutate(pathway = pathway %>% str_remove("^GOBP_") %>% str_replace_all("_", " ") %>% str_to_lower())   #%>% str_trunc(width = 100))

# RevyGO on iNeurons
revy_list <- list()
for (target_gene in names(gsea_list)) {
  df <- gsea_list[[target_gene]]
  df <- df %>% dplyr::filter(cell_type == "iNeuron") %>% dplyr::mutate(pathway = pathway %>% str_remove("^GOBP_") %>% str_replace_all("_", " ") %>% str_to_lower()) 
  goterms <- Term(GOTERM)
  go_map <- data.frame(GOID=names(goterms), pathway=goterms)
  df <- merge(x = df, y = go_map, by = "pathway", all.x = TRUE)
  df <- df[!is.na(df$GOID), ]
  df <- df %>% dplyr::arrange(padj)
  
  # Revygo Similarity
  simMatrix <- calculateSimMatrix(df$GOID,
                                  orgdb="org.Hs.eg.db",
                                  ont="BP",
                                  method="Rel")
  scores <- setNames(-log10(df$padj), df$GOID)
  reducedTerms <- reduceSimMatrix(simMatrix,
                                  scores,
                                  threshold=0.7,
                                  orgdb="org.Hs.eg.db")
  reducedTerms <- reducedTerms %>% dplyr::arrange(desc(score)) %>% dplyr::select(go, parent, score, parentTerm)
  colnames(reducedTerms) <- c("GOID", "parent_GOID", "score", "parentTerm")
  reduced_with_direction <- merge(df, reducedTerms, by = "GOID", all.x = TRUE)
  
  # Section of top 10 up and downregulated pathways
  top_10_up <- reduced_with_direction %>%
    filter(NES > 0) %>%
    arrange(padj) %>%
    distinct(parentTerm, .keep_all = TRUE) %>% 
    head(10)
  
  # 3. Identify the Top 10 DOWN-regulated parent terms
  top_10_down <- reduced_with_direction %>%
    filter(NES < 0) %>%
    arrange(padj) %>%
    distinct(parentTerm, .keep_all = TRUE) %>% 
    head(10)
  
  # 4. Combine for your final "Top 20" list
  final_terms_df <- bind_rows(top_10_up, top_10_down)
  revy_list[[target_gene]] <- final_terms_df
}
```

```{r}
revy_df <- dplyr::bind_rows(revy_list)
gene_filt <- c("KMT2A", "KMT2B", "KMT2C", "SETBP1", "SETD1B")
revy_df <- revy_df[revy_df$target_gene %in% gene_filt] %>% dplyr::filter(padj < 0.01)
revy_df <- gsea_results_full[gsea_results_full$pathway %in% revy_df$parentTerm] %>% dplyr::filter(target_gene %in% gene_filt) %>% dplyr::filter(cell_type == "iNeuron")
ndd_pathway_vector <- c(
  # --- 1. Epigenetic & Early Fate ---
  "methylation",                                     # Direct link to COMPASS/H3K4me
  "neuron fate commitment",                          # Essential for iNeuron transition
  "stem cell proliferation",                         # Neurogenesis/progenitor stage
  "pattern specification process",                   # Early brain layout
  "diencephalon development",                        # Regional brain specificity
  
  # --- 2. Structural Maturation & "Wiring" ---
  "cell morphogenesis involved in neuron differentiation", 
  "axon development",
  "neuron projection guidance",
  "cranial nerve development",
  "apoptotic process involved in development",       # Corrective pruning/cell death
  
  # --- 3. Synaptic Assembly & Adhesion ---
  "synapse organization",
  "regulation of synapse structure or activity",
  "protein localization to synapse",                 # Mechanistic/trafficking
  "cell junction organization",                      # Synaptic stability/adhesion
  
  # --- 4. Functional Signaling & Ion Flux ---
  "synaptic signaling",
  "neurotransmitter transport",                      # Functional output
  "neuropeptide signaling pathway",
  "regulation of membrane potential",
  "potassium ion transport",
  "monoatomic ion transport"
)
#revy_df <- revy_df[revy_df$pathway %in% ndd_pathway_vector, ]
revy_df <- revy_df %>% dplyr::mutate(pathway = pathway %>%  str_trunc(width = 40))
```
...
#gsea_results$pathway <- gsub(pattern = "Reactome ", replacement = "", x = gsea_results$pathway)
#sig_paths <- gsea_results %>% dplyr::filter(padj < 0.05) %>% dplyr::pull(pathway)
...
#png(filename = "integrative_figures/iNeuron_GSEA_pathway_heatmap_GOBP.png", width = 2500, height = 2500, units = "px", res = 300)
pdf(file = "integrative_figures/iNeuron_GSEA_pathway_heatmap_GOBP.pdf", width = 7, height = 7)
pheatmap(
  as.matrix(nes_mat),
  color = my_color,
  breaks = my_breaks,
  ...
  clustering_distance_rows = "euclidean",
  clustering_distance_cols = "euclidean",
  main = "iNeuron GSEA Pathway Enrichment"
)
dev.off()
```
